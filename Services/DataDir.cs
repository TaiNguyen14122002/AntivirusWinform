...
namespace ScanAndRemoveVirus.Services
{
    ...
    /// Mặc định: AppData\ ngay tại gốc repo (tự dò bằng thư mục TestSamples\ ở gốc repo,
    /// cùng kiểu leo-cây với TestSamples.FolderPath) — dữ liệu đi theo repo, dễ thấy/dễ dọn.
    /// Không thấy TestSamples\ (chạy ngoài repo) -> fallback %AppData%\ScanAndRemoveVirus.
    ...
    internal static class DataDir
    {
        ...
        private static string Root
        {
            get
            {
                ...
                {
                    if (Directory.Exists(Path.Combine(dir.FullName, "TestSamples")))
                    {
